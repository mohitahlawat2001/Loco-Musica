...
const cart = () => {
    ...
    return (
        <div className="mx-2 px-2">
            ...
            <button className="bg-red-500 text-white px-4 py-2 rounded-md mb-4"
                onClick={handleClearCart}>Clear Cart</button>
            <div className="grid grid-cols-3 gap-4">
                {cart.map((item) => (
                    <div key={item?.card?.info?.id} className="bg-white shadow-md p-4 rounded-md">
                        <img src={'https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,h_600/'+item?.card?.info?.imageId} alt={item?.card?.info?.name} className="w-32 h-32 object-cover rounded-md mb-2" />
